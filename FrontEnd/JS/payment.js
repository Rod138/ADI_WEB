...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function autoFillExpectedAmount() {
        ...
        const amountWithSurcharge = calculateExpectedAmountWithLateFee(quota.amount, year, monthInput.value);
        amountExpectedInput.value = Number.isFinite(amountWithSurcharge)
            ? amountWithSurcharge.toFixed(2)
            : '';

        amountExpectedInput.title = isLatePayment(year, monthInput.value)
